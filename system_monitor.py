...
class Application(ttk.Frame):
	...
	## LOADS
        ...
        #
        self.load_frame.grid(column=0, row=7, columnspan=4, sticky="wens")
        # label
        core_load_lbl = ttk.Label(self.load_frame, text="Cpu loads", anchor="center")
        core_load_lbl.grid(column=0, row=0, columnspan=4, sticky="wens")
        #
        #
        self.cbl_var = tk.IntVar()
        self.cbl = ttk.Checkbutton(self.load_frame, text="On/Off", variable=self.cbl_var, command=self.fcbl)
        # 1 selected
        self.cbl_var.set(0)
        ##
        #core_real = psutil.cpu_count(logical=False)
        ...
        #num_row = int(core_real/5)
            ...
            #num_row += 1
        ##
        self.cbl.grid(column=4, row=1, rowspan=num_row)
        # list of StringVar of label: loads
        self.load_list = []
        # label list
        self.load_label = []
        #
        ...
        for ii in range(core_real):
            ...
            self.load_var.set("0")
            self.load_list.append(self.load_var)
            self.load_lbl = ttk.Label(self.load_frame, textvariable=self.load_var)
            self.load_label.append(self.load_lbl)
        #
        ii = 0
        # 
        for rrow in range(num_row):
            for ccolumn in range(4):
                ...
                if ii == core_real:
                    break
                #
                self.load_label[ii].grid(column=ccolumn, row=rrow+1)
                self.load_frame.columnconfigure(ccolumn, weight = 1)
                ii += 1
        #
        ######################## GPU Nvidia
        ## LOAD
    ...
    def pop_load(self):
        ...
        if self.cbl_var.get():	
            if self.p_btn:
                ...
                dcpul.append(cpu_load)

                self.master.after(LOOP_INTERVAL, self.pop_load)
                ...
                if c_point == deque_size:
                    ...
                    if self.cbl_var.get():
                        for item in self.load_list:
                            item_idx = self.load_list.index(item)
                            try:
                                item.set(str(dcpul[c_point][item_idx]))
                            except:
                                item.set("0")
        # CPU TEMPERATURES
        elif n == 1:
